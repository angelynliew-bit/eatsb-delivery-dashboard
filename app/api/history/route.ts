...
export async function GET() {
  try {
    ...
    if (!context.permissions.canViewHistory && !context.permissions.canViewExceptions && !context.permissions.canViewMasterData) {
      return NextResponse.json({ error: "You do not have permission to view history." }, { status: 403 });
    }
    ...
    return NextResponse.json({
      logs: context.permissions.canViewHistory ? (logs.data ?? []).map((log: any) => ({
        ...log,
      ...
      exceptions: context.permissions.canViewExceptions ? (exceptions.data ?? []).map((exception: any) => ({
        ...exception,
      ...
      masterData: context.permissions.canViewMasterData ? {
      ...
      } : { channels: [], chains: [], stores: [], products: [] },
    });
  } catch (error) {
