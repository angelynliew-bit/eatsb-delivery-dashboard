...
export default function DashboardShell({ initialSection = "Overview" }: { initialSection?: string }) {
  ...
  async function loadHistory() {
    if (!me?.permissions.canViewHistory && !me?.permissions.canViewMasterData && !me?.permissions.canViewExceptions) return;
  ...
  const isAnalysisSection = ["Channel Analysis", "Chain Analysis", "Product Analysis", "Store Analysis"].includes(activeSection);

  return (
    <div className="app-frame">
      {me && <SideNavigation email={me.user.email} name={me.user.name} role={me.role} permissions={me.permissions} />}
      <main className="app-shell">
      <header className="topbar">
        <div>
          <p className="eyebrow">East Asian Traders</p>
          <h1>{activeSection === "Overview" ? "Order and Delivery Tracking Dashboard" : activeSection}</h1>
        </div>
      ...
      {!loading && dashboard && activeSection === "Overview" && (
      ...
      {activeSection === "Spreadsheet Import" && (
        <section className="view">
      ...
      {!loading && dashboard && isAnalysisSection && (
        <section className="view">
          <div className="grid-two">
            <section>
      ...
      {!loading && dashboard && activeSection === "Delivery Records" && (
        <section className="view">
      ...
      {activeSection === "Import Exceptions" && (
        <section className="view">
          <section>
      ...
      {activeSection === "User Management" && (
        <section className="view">
          <div className="empty">Administrator-only user management. Create or invite users in Supabase, then assign their role in the profiles table.</div>
        </section>
      )}

      {activeSection === "Settings" && (
        <section className="view">
          <div className="empty">Settings are available for authorised users. Authentication and role enforcement are active.</div>
        </section>
      )}
      </main>
    </div>
