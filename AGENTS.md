# eatsb-delivery-dashboard

Internal delivery tracking dashboard for East Asian Traders — imports spreadsheet delivery data, shows weekly/monthly delivery counts and units by channel, chain, store, and product, with drill-down analysis and CSV export.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "238495036+angelynliew-bit@users.noreply.github.com"
  git config user.name "angelynliew-bit"
  ```
